...
# ci is what the managed go.yml workflow invokes (make ci inside the go gate
# image). This module's check already includes the race-enabled tests and has
# no coverage/release stages, so ci is exactly check.
.PHONY: ci
ci: check
